...
public class NameValueEdit extends RelativeLayout{
	...
	private void setNameTextView(TypedArray a){
		...
		int nameTextSize = a.getDimensionPixelSize(R.styleable
				.NameValueEdit_nameSize, DefaultTextSize);
		if(nameTextSize>0)
...
//		if(nameTextColor!=-1)
	...
	private void setValueEditView(TypedArray a){
		...
		int size = a.getDimensionPixelSize(R.styleable
				.NameValueEdit_valueSize, DefaultTextSize);
		if(size>0)
			valueEditView.setTextSize(TypedValue.COMPLEX_UNIT_PX,size);
		int color = a.getColor(R.styleable
				.NameValueEdit_valueColor, -1);
		int hintColor = a.getColor(R.styleable
				.NameValueEdit_hintColor, -1);
		int inputType = a.getInt(R.styleable
				.NameValueEdit_editInputType, -1);
		if(inputType > 0) {
			valueEditView.setInputType(inputType);
		}
//		if(color!=-1)
