...
public class NumberUtils {
     ...
     * 比如将'0101'转为5
     * @param val
     * @return 如果传入了空值或者非法字符，会直接返回0

    public static int byteString2Int(String val) {
        if(val==null || val.length()<1)
            return 0;
        char[] binarys = val.toCharArray();
        int position = 1;
        int result = 0;
        for(int i=binarys.length-1;i>=0;i--){
            if('1'==binarys[i])
                result += position;
            else if('0'!=binarys[i])//出现了既不是0也不是1的字符，直接返回0
                return 0;
            position = position*2;Integer.toBinaryString()
        }
        return result;
    }*/
}
